...
class IndieAuthSettingsForm extends ConfigFormBase {
  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
    ...
    $form['login'] = [
      '#type' => 'fieldset',
      '#title' => $this->t('Login'),
      '#description' => $this->t('Allow users to login into this site by using their domain. A "Web Sign-In" block is available where users can enter their domain to login.<br />After authentication a new user account will be created if this domain does not exist yet. The account will automatically be verified.<br />Authenticated users can use the same block to map an existing domain with their account.<br />Users who login with their domain will not be able to change the username, password or set an e-mail address (yet).')
    ];
