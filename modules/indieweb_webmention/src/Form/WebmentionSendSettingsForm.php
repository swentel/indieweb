...
class WebmentionSendSettingsForm extends ConfigFormBase {
  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
    ...
    $form['syndication_targets_wrapper']['syndication_targets'] = [
      ...
      '#description' => $this->t('Enter every target line by line if you want to publish content, in following format:<br /><br />Name|webmention_url|selected|extra class<br />- selected is optional. Set to 1 so the target is default selected on the node form.<br />- extra class is optional. Add a custom class to the link.<br /><br /><strong>Example</strong><br />Twitter (bridgy)|https://brid.gy/publish/twitter<br />Fediverse|https://fed.brid.gy/<br />IndieNews|https://news.indieweb.org/en|0|u-syndication<br /><br />When you add or remove targets, extra fields will be enabled on the manage display screens of every node type (you will have to clear cache to see them showing up).<br />These need to be added on the page (usually on the "full" view mode) because bridgy will check for the url in the markup, along with the proper microformat classes.<br />The field will print them hidden in your markup, even if you do not publish to that target, that will be altered later.<br />You can also add them yourself:<br /><div class="indieweb-highlight-code">&lt;a href="https://brid.gy/publish/twitter"&gt;&lt;/a&gt;</div><br />These targets are also used for the syndicate-to request if you are using micropub.<br />Consult the README file that comes with this module if you want to integrate with the Fediverse.')
    ];
