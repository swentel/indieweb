...
class MicropubSettingsForm extends ConfigFormBase {
  ...
  protected function getPostTypes() {
    $post_types = [
      ...
      'repost' => [
        'description' => $this->t("A repost request contains a URL in 'repost-of' and 'h' value is 'entry'. In case content is stored, the link will be rendered as a quotation by the formatter."),
        'optional_body' => TRUE,
        'link_field' => TRUE,
        'send_webmention' => TRUE,
      ],
  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
    ...
    foreach ($this->getPostTypes() as $post_type => $configuration) {
      ...
      $form[$post_type][$post_type . '_upload_field'] = [
        ...
        '#description' => $this->t('Select the field which will be used to store images. Make sure the field exists on the node type.<br />Only images are allowed, multiple if the field has a cardinality other than 1.'),
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    foreach ($this->getPostTypes() as $post_type => $configuration) {
     ...
     $config->set($post_type . '_create_node', $form_state->getValue($post_type . '_create_node'))
        ...
        ->set($post_type . '_audio_field', $form_state->getValue($post_type . '_audio_field'))
        ->set($post_type . '_video_field', $form_state->getValue($post_type . '_video_field'))
